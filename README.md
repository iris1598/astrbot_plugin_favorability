...
| `/重置好感度` | 重置自己的好感度记录（关系回到「普通朋友」） |
| `/确认关系` | 接受 AI 待确认的关系升降提议 |
| `/取消关系` | 拒绝 AI 待确认的关系升降提议 |

> 群里用 `@机器人` 唤醒后直接发送指令即可。唤醒用的 `@机器人` 与 `@全体成员` 不会被当作查询/操作目标：
> `@机器人 查询好感度` 查的是你自己，`@机器人 查询好感度 @某人` 查的才是某人。
...
2. Bot 向用户发送确认提示，例如：`💞 TA 想和你们的关系更进一步：「普通朋友」→「熟络好友」`；
...
| 档位 | 行为准则概要 | 卡片强调色 |
|---|---|---|
| 挚爱恋人 | 专属伴侣，唯一且偏爱，甜蜜陪伴 | 粉红 |
| 知心挚友 | 托付心声，深厚信任，分担忧愁与温暖关照 | 珊瑚橙 |
| 熟络好友 | 经常往来，轻松友好，接梗愉快且有默契 | 青绿 |
| 普通朋友 | 客气平衡，礼貌正常交流（默认档位） | 灰蓝 |
| 生疏之交 | 生疏客套，公事公办，克制并保持明确界限 | 琥珀黄 |
| 不合对头 | 理念对立，界限森严，严肃拒绝无理要求 | 红色 |
| 决裂陌路 | 彻底决裂，互不搭理，收敛至最低限度应答 | 紫色 |
...
- **v3.0.1**：修复群里「`@机器人 查询好感度`」被误解析为查询机器人自己的问题——@机器人（唤醒前缀）与 @全体成员 不再被当作查询/操作目标；管理员指令同步修复
